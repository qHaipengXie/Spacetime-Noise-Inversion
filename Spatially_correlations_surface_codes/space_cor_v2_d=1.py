...
#     if len(neighbordic[error_coords[i]])==24 and len(neighbordic[error_coords[i+1]])==24:
#         qubit1 = error_coords[i]
#         qubit2 = error_coords[i+1]
#         break
qubit1 = (8,4,0)
qubit2 = (10,4,0)
indice1 = map_coords_to_idx[qubit1]
indice2 = map_coords_to_idx[qubit2]
error_coords2 = [qubit1,qubit2]
for p0 in plist:
    # p0 = 0.02
    p_c = 0.1
    error_all = []
    for i in range(shots):
        ...
        for c in error_coords:
            ...
            # two_flip = np.random.binomial(1,p_c,size=24)
            # for k in range(len(neighbordic[c])):
            #     if two_flip[k] ==1:
            #         error[map_coords_to_idx[c]] = not error[map_coords_to_idx[c]]
            #         point = neighbordic[c][k]
            #         error[map_coords_to_idx[point]] = not error[map_coords_to_idx[point]]
            #     else:
            #         None
            # flag = sum(two_flip[len(neighbordic[c]):])%2
            # if flag ==1:
            #     error[map_coords_to_idx[c]] = not error[map_coords_to_idx[c]]
            # else:
            #     None
    ...
    sub_matrix = error_all[:, [indice1, indice2]]

    # 计算每一列为 True 的平均值（即 True 的比例）
    mean_col1 = np.mean(sub_matrix[:, 0])
    mean_col2 = np.mean(sub_matrix[:, 1])

    # 计算乘积
    product = mean_col1 * mean_col2

    # 计算两列同时为 True 的概率
    both_true_prob = np.mean(np.logical_and(sub_matrix[:, 0], sub_matrix[:, 1]))

    # 打印结果
    # print("第一列为True的平均值：", mean_col1)
    # print("第二列为True的平均值：", mean_col2)
    # print('概率： ',p0)
    # print("两者乘积：", product)
    # print("两列同时为True的概率：", both_true_prob)
    p1p2.append(product)
    p12.append(both_true_prob)
print(p1p2)
print(p12)
#     import pymatching
